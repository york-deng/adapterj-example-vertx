...
public class SourceQuery implements POJOQuery<Source> {

	private static final boolean DEBUG = Debugger.DEBUG;
	
	private static final String TAG = SourceQuery.class.getName();

	private static SourceQuery instance = new SourceQuery();
	 ...
	 * Constructors
	 */
	public SourceQuery(final OrmOpenHelper helper) {
		_helper = helper;
	}
	 ...
	 * @return
	 */
	public static SourceQuery getInstance() {
		return (instance);
	}
	 ...
	 * @return
	 */
	public static SourceQuery getInstance(final OrmOpenHelper helper) {
		return (instance = new SourceQuery(helper));
	}

	public Source findFirstByAttr(final String attrName, final String attrValue) {
		if (attrName == null) {
	...
	public Source findByMatching(final Source parameter) {
		if (parameter == null) {
			final StackTraceElement t = (new Throwable()).getStackTrace()[0];
		...
		try {
			...
			final Where<Source, Long> where = builder.where().isNotNull(Source.COLUMN_ID);
			if (parameterId != null && parameterId > 0) where.and().eq(Source.COLUMN_ID, parameterId);
			if ((parameterId == null || parameterId <= 0) && domain != null) where.and().eq(Source.COLUMN_DOMAIN, domain);
			if ((parameterId == null || parameterId <= 0) && www != null) where.and().eq(Source.COLUMN_WWW, www);
			...
			final String g = String.format(f, t.getFileName(), t.getLineNumber(), t.getMethodName(), parameter.toJSONString());
			Log.i(TAG, g, e);
	 ...
	 */
	public Source create(final Source source, final Version version) throws QueryException {
		if (source == null) {
			final StackTraceElement t = (new Throwable()).getStackTrace()[0];
		...
		final Callable<Source> callable = new Callable<Source>() {
			@Override
			public Source call() throws SQLException { // Make sure that if either one fails, the transaction is rolled back
				final Date now = new Date();
				source.setVersion(version.getVersion());
				source.setInsertTime(now);
				source.setUpdateTime(now);
				source.setState(Source.STATE_SUBSCRIBED);
				
				final Dao<Source, Long> dao1 = _helper.getDao(Source.class);
				dao1.create(source);
				dao1.refresh(source);
				
				final Version newVersion = new Version();
				newVersion.setSource(source);
				newVersion.setType(Version.TYPE_SOURCE);
				...
				final Dao<Version, Long> dao2 = _helper.getDao(Version.class);
	 ...
	 * @throws QueryException
	 */
	public Source update(final Source parameter) throws QueryException {
		if (parameter == null) {
			final StackTraceElement t = (new Throwable()).getStackTrace()[0];
		...
		Source returnObject = null;
		try {
			final Long id = parameter.getId();

			final Dao<Source, Long> dao = _helper.getDao(Source.class);
			final QueryBuilder<Source, Long> builder = dao.queryBuilder();
			final Where<Source, Long> where = builder.where();
			if (id != null) where.eq(Source.COLUMN_ID, id);
			else where.eq(Source.COLUMN_DOMAIN, domain).or()
				      .eq(Source.COLUMN_WWW, www);
	...
	@Override
	public boolean exists(final Source parameter) throws QueryException {
		if (parameter == null) {
			final StackTraceElement t = (new Throwable()).getStackTrace()[0];
			final String f = "(%s:%d) %s: parameter is null";
			final String g = String.format(f, t.getFileName(), t.getLineNumber(), t.getMethodName());
		...
		try {
			final Dao<Source, Long> dao = _helper.getDao(Source.class);
			final QueryBuilder<Source, Long> builder = dao.queryBuilder();
			if (name != null) builder.where().eq(Source.COLUMN_NAME, name);
			else if (domain != null) builder.where().eq(Source.COLUMN_DOMAIN, domain);
			else if (www != null) builder.where().eq(Source.COLUMN_WWW, www);
			else return (false);
	...
	@Override
	public Source refresh(Source parameter) throws QueryException {
		if (parameter == null) {
			final StackTraceElement t = (new Throwable()).getStackTrace()[0];
			final String f = "(%s:%d) %s: parameter is null";
			final String g = String.format(f, t.getFileName(), t.getLineNumber(), t.getMethodName());
	...
	@Override
	public Source createOrUpdate(final Source pojo) throws QueryException {
		final String s = "You can NOT call createOrUpdate(Source) method to INSERT or UPDATE a source record, " +
				         "but you CAN call create(Source, Version) method to INSERT a new source record, or "   + 
				         "call update(Source) method to UPDATE a source record. ";
	...
	@Override
	public int delete(final Source pojo) {
		throw new NotImplementedException("...");
	}
}
